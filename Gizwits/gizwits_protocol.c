...
uint32_t ICACHE_FLASH_ATTR calculateDynamicBitPartLen(dataPointFlags_t *aFlag)
{
    ...
    if(0x01 == aFlag->flagLEDonoff)
    {
        bitFieldBitLen += LEDonoff_LEN;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(uint8_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagLEDonoff)
    {
        info->event[info->num] = EVENT_LEDonoff;
        info->num++;
        dataPoints->valueLEDonoff = gizVarlenDecompressionValue(bitFieldOffset,LEDonoff_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += LEDonoff_LEN;
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueLEDonoff != cur->valueLEDonoff)
    {
        ...
        gizwitsProtocol.waitReportDatapointFlag.flagLEDonoff = 1;
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    /*** Fill the bit ***/
    if(gizwitsProtocol.waitReportDatapointFlag.flagLEDonoff)
    {
        gizVarlenCompressValue(bitFieldOffset,LEDonoff_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valueLEDonoff);
        bitFieldOffset += LEDonoff_LEN;
    }
